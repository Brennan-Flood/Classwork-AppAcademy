...
function reverseString(str) {
  ...
  newStr += strArr[strArr.length - 1] + reverseString(strArr.slice(0, strArr.length - 1).join(""));
  return newStr;
}
...
function flatten(data) {
  ...
  data.forEach((ele) => {
    if (ele instanceof Array) {
      returnArr = returnArr.concat(flatten(ele));
    } else {
      returnArr.push(ele);
    }
  })
  ...
  Object.keys(directories).forEach((key) => {
    if (key === targetFile) {
      ...
      return;
       ...
       nextPath = pathFinder(directories[key], targetFile);
       if (nextPath) {
         filePath += key + nextPath;
         found = true;
         return;
       }
    }
